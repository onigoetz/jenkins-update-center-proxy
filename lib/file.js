...
export const cache = {
  ...
  async set(key, value, ttl) {
    ...
    if (isCacheable(key)) {
      ...
      // Rename is atomic within the directory: a concurrent `get` sees either no
      // entry or a complete one. Writing in place lets it read a JSON prefix.
      const partial = `${destination}.${randomUUID()}.partial`;
      await fsPromises.writeFile(partial, value);
